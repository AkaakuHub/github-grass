...
def generate_svg(contribution_data):
    ...
    # 最大30週間分のデータを取得
    rest_weeks = weeks_count - 30 if weeks_count > 30 else weeks_count
    count = 0

    # 背景に、グレーでrectを描画
    dwg.add(
        dwg.rect(insert=(0, 0), size=(12 * 30 + 2, 84 + 2), fill="#ffffff", rx=2, ry=2)
    )
    ...
    github_mark = """
    <circle cx="175" cy="42" fill="#ffffff" r="39" />
    <path fill-rule="evenodd" clip-rule="evenodd" transform="scale(0.8, 0.8) translate(170, 3)"
